...
const OrderSummary = () => {
	...
	//   CheckoutContext
	// );
	const { orderSummaryCurrency, orderSummaryMiles, isCurrency, tipAmount } = useContext(CheckoutContext);
	const { exceptionAmount, showException } = useContext(VoucherContext);

	totalBeforeTaxNEW = (totalBeforeTaxNEW * 100 - exceptionAmount * 100) / 100;
	...
	const formattedTotal = isCurrency ? formatCurrency(orderSummaryCurrency) : formatMiles(orderSummaryMiles);
	console.log(`formattedTotal - ${JSON.stringify(formattedTotal)}`);
	console.log(`TEST`);

	// console.log(`voucherData - ${JSON.stringify(voucherData.amount)}`);

	const { subtotal, totalBeforeTax, tax, total } = formattedTotal;
	...
	return (
		<View style={styles.orderSummaryContainer}>
			...
			<View style={styles.rowContainer}>
				...
				<Text style={styles.textLabel}>{tipAmount}</Text>
			</View>
			<View style={styles.rowContainer}>
				<Text style={styles.textLabel}>{totalBeforeTaxLabel}</Text>
				<Text style={styles.textLabel}>{totalBeforeTaxNEW}</Text>
			</View>
			<View style={styles.rowContainer}>
			...
			<View style={styles.rowContainer}>
				...
				<Text style={[styles.orderTotal, styles.orderTotalAmount]}>{total}</Text>
			</View>
		</View>
	);
};
