...
let instructionsText = 'Please enter your voucher information below and tap Submit to continue.';
let translations = {
...
const handleExpirationeDate = expDate => {};
...
const VoucherModal = ({ modalVisible, setModalVisible }) => {
	...
	const handleInputChange = (name, value) => {
		console.log(`TMB - name, value - ${JSON.stringify(name)} - ${JSON.stringify(value)}`);
		setData({
			...data,
			[name]: value,
		});
	...
	const validateForm = () => {
		...
		// console.log(`validateForm numberFlag- ${JSON.stringify(numberFlag)}`);
		// console.log(`validateForm numberFlag data - ${JSON.stringify(data)}`);

		let amountFlag = validateVoucherAmount(data.amount);
		// console.log(`validateForm amountFlag- ${JSON.stringify(amountFlag)}`);
		// console.log(`validateForm amountFlag data - ${JSON.stringify(data)}`);

		let expireFlag = validateExpireDate(data.expireDate);
		// console.log(`validateForm expireFlag- ${JSON.stringify(expireFlag)}`);
		// console.log(`validateForm expireFlag data - ${JSON.stringify(data)}`);

		let cvvFlag = validateCVV(data.cvv);
		// console.log(`validateForm cvvFlag- ${JSON.stringify(cvvFlag)}`);
		// console.log(`validateForm cvvFlag data - ${JSON.stringify(data)}`);
		console.log(
			`numberFlag && amountFlag && expireFlag && cvvFlag - ${numberFlag} && ${amountFlag} && ${expireFlag} && ${cvvFlag}`
		);
		console.log(`TMB - validateForm - ${JSON.stringify(data)}`);
	...
	const validateVoucherNumber = voucherNum => {
		console.log(`data - validateVoucherNumber - voucherNum.length - ${JSON.stringify(voucherNum.length)}`);

		if (voucherNum.length < 19) {
			console.log(`data - validateVoucherNumber - false - before setData- ${JSON.stringify(data)}`);
			let newObj = {
				...data,
				numberError: 'Please Enter Valid Voucher Number',
			};
			console.log(`newObj -false - validateVoucherNumber - ${JSON.stringify(newObj)}`);
			setData(prevState => ({
				...prevState,
				numberError: 'Please Enter Valid Voucher Number',
			}));
			console.log(`data -false - validateVoucherNumber - ${JSON.stringify(data)}`);
			return false;
		}
		console.log(`data - true - validateVoucherNumber - ${JSON.stringify(data)}`);
		return true;
	};
	const validateVoucherAmount = voucherAmount => {
		if (voucherAmount.length === 0) {
			console.log(
				`data - validateVoucherAmount - voucherAmount.length -  ${JSON.stringify(voucherAmount.length)}`
			);
			console.log(`data - validateVoucherAmount - false - before setData - ${JSON.stringify(data)}`);
			setData(prevState => ({
				...prevState,
				amountError: 'Please Enter Voucher Amount',
			}));
			console.log(`data - validateVoucherAmount - false - after setData - ${JSON.stringify(data)}`);
			return false;
		}
		// console.log(
		//   `data - voucherAmount.length -  ${JSON.stringify(voucherAmount.length)}`,
		// );
	...
	const validateExpireDate = expireDate => {
		if (expireDate.length === 0) {
			console.log(`data - validateExpireDate - expireDate.length -  ${JSON.stringify(expireDate.length)}`);
			console.log(`data - validateExpireDate - false - before setData - ${JSON.stringify(data)}`);
			setData(prevState => ({
				...prevState,
				expireDateError: 'Please Enter Valid Expiration Date',
			}));
			console.log(`data - validateExpireDate - false - after setData - ${JSON.stringify(data)}`);
			return false;
		}
		return true;
	};
	const validateCVV = cvv => {
		if (cvv.length === 0) {
			console.log(`data - validateCVV - cvv.length -  ${JSON.stringify(cvv.length)}`);
			console.log(`data - validateCVV - false - before setData - ${JSON.stringify(data)}`);
			setData(prevState => ({
				...prevState,
				cvvError: 'Please Enter Valid CVV',
			}));
			console.log(`data - validateCVV - false - after setData - ${JSON.stringify(data)}`);
			return false;
		}
		return true;
	};
	...
	console.log(
		`data.numberError.length - ${data.numberError.length} - data.amountError.length - ${data.amountError.length} - data.expireDateError.length - ${data.expireDateError.length} - data.cvvError.length - ${data.cvvError.length}`
	);
	console.log(`data - ${JSON.stringify(data)}`);
	return (
		<Modal
		...
		>
			<KeyboardAvoidingView style={styles.voucherContainer} behavior='padding' enabled>
				{/* <View style={styles.voucherContainer}> */}
				<View
					style={[
						styles.contentContainer,
						orientation === 'portrait' ? styles.contentContainerPortrait : styles.contentContainerLandscape,
					]}
				>
					{isVoucherForm ? (
						<View>
							...
							>
								<View style={orientation === 'landscape' ? styles.voucherNumContainerLandscape : null}>
									...
									<TextInput
										...
										onChangeText={text => handleInputChange('number', formatVoucherNumber(text))}
										value={data.number}
										placeholder='1234-5678-9123-4567'
										maxLength={19}
										keyboardType='number-pad'
								...
								>
									<View style={[styles.rowItem, { marginRight: 13 }]}>
										...
										<TextInput
											...
											onChangeText={text => handleInputChange('amount', toCurrency(text))}
											value={data.amount}
											placeholder='$0.00'
											maxLength={8}
											keyboardType='number-pad'
										...
										<View style={{ height: 40 }}>
											<Text style={styles.errorMessage}>{data.amountError}</Text>
										</View>
									</View>
									<View style={[styles.rowItem, { marginRight: 13 }]}>
										...
										<TextInput
											...
											onChangeText={text => handleInputChange('expireDate', formatExpDate(text))}
										...
										/>
										<View style={{ height: 40 }}>
											<Text style={styles.errorMessage}>{data.expireDateError}</Text>
										</View>
									</View>
						...
						<View>
							<View style={styles.headingContainer}>
								<Text style={styles.headingText}>{voucherAppliedHeadingText}</Text>
							</View>
							<View style={styles.instructionTextContainer}>
								...
								<View style={[styles.footerContainer, { marginTop: 100 }]}>
									<View sytle={styles.submitButtonContainer}>
										...
										>
											<Text style={styles.submitButtonText}>Apply Another Voucher</Text>
										</TouchableHighlight>
									</View>
...
const styles = StyleSheet.create({
	...
	isFocused: {
		borderColor: '#0070c9',
		backgroundColor: white,
		//boxShadow: "0 0 0 0.17647rem rgba(131,192,253,.5)",
		shadowOpacity: 0.75,
		shadowRadius: 5,
		shadowColor: 'blue',
